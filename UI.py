...
def deshboard():
    ...
    label_text =ttk.Label(width=70, font=("Arial", 12))
    label_text.place(x=600,y=300)
    
    def text():
        try:
            texts=ocr.Text("meme.jpg")
            print(filename)
            label_text.config(text=texts)
        except Exception as es:
            messagebox.showerror("Error!" , f"Error Due to : {str(es)}", parent = des)

    btn_audio = ttk.Button(text='Text Extraction', command=text)
    btn_audio.place(x=450,y=300)
