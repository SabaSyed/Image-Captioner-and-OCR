...
def deshboard():
    ...
    def img_insert():
        ...
        label=Label(des,image=img)
        label.place(x=80,y=75)    
        

    des = Tk()
    des.title("Home BeMyVision")	
    des.maxsize(width=1000 ,  height=500)
    des.minsize(width=1000 ,  height=500)	
    ...
    f.place(x=80,y=75)
    btn_insert= Button(des, text = "Insert Image" ,font='Verdana 10 bold', width = 20, command = img_insert)
    btn_insert.place(x=150, y=440)
    ...
    label3.place(x=600,y=218)
   
    def translatelang(event):
        lang=options[selected.get()]
        result = translator.translate(cap, dest=lang)
