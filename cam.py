...
import testing_caption_generator2 as testing
import time
import cv2 
...
def deshboard():
    ...
    loading_frame=Frame(des, bg='#FFF',width=400, height=50)
    ...
    pb.place(x=100,y=5)
    loading_label = Label(loading_frame,text="Please Wait", pady=5, bg="#FFF",fg='#2e215f')
    ...
    def translatelang(event):
        #loading_frame.place(x=700,y=500)
        #pb.start(20)
        try:
    ...
    label_text =Label(f_frame, font = 'Verdana 12', wraplengt=400 ,width=40,height=5, bg="#FFF", fg='#2e215f' , borderwidth=1, relief="solid")
    ...
    def img_insert():
        ...
        def generate_caption():
            ...
            btn_insert["state"] = "normal"
            btn_audio["state"] = "normal"
            lang_cb["state"] = "readonly"
        ...
        try:
            ...
            messagebox.showerror("Error#404" , f"Image not found.", parent = des)
        if os.path.isfile(filename):
            loading_frame.place(x=700,y=540)
            pb.start(20)
            thread=threading.Thread(target=generate_caption)

            thread.start()        

            btn_capture["state"] = "disabled"
            btn_insert["state"] = "disabled"
            btn_audio["state"] = "disabled"
    ...
    btn_capture= Button(f_frame,text = "Capture Image" , width = 15, command = captureimg, fg='#FFF' , bg='#2e215f',pady=6)
    btn_capture.place(x=270, y=520)
